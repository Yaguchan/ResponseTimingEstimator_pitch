...
def get_offset2(df):
    ...
    new_df = {'start':[], 'end':[], 'spk':[], 'da':[], 'da1':[], 'spk_change':[], \
            'nxt_start':[], 'nxt_end':[], 'nxt_spk':[], 'nxt_da':[], 'nxt_da1':[], }
    
    new_df['start'].append(df['start'].iloc[0])
    new_df['end'].append(df['end'].iloc[0])
    new_df['spk'].append(df['spk'].iloc[0])
    new_df['da'].append(df['da'].iloc[0])
    new_df['da1'].append(df['da1'].iloc[0])
    
    pre = df['spk'].iloc[0]
    pre_end = df['end'].iloc[0]
    
    for i in range(1, len(df)):
        
        if df['end'].iloc[i] > pre_end:
            
            new_df['start'].append(df['start'].iloc[i])
            new_df['end'].append(df['end'].iloc[i])
            new_df['spk'].append(df['spk'].iloc[i])
            new_df['da'].append(df['da'].iloc[i])
            new_df['da1'].append(df['da1'].iloc[i])
            new_df['nxt_start'].append(df['start'].iloc[i])
            new_df['nxt_end'].append(df['end'].iloc[i])
            new_df['nxt_spk'].append(df['spk'].iloc[i])
            new_df['nxt_da'].append(df['da'].iloc[i])
            new_df['nxt_da1'].append(df['da1'].iloc[i])
            
            if pre != df['spk'].iloc[i]:
                new_df['spk_change'].append(1)
            else:
                new_df['spk_change'].append(0)
            
            # 発話の更新
            pre = df['spk'].iloc[i]
            pre_end = df['end'].iloc[i]
                
    new_df['start'] = new_df['start'][:-1]
    new_df['end'] = new_df['end'][:-1]
    new_df['spk'] = new_df['spk'][:-1]
    new_df['da'] = new_df['da'][:-1]
    new_df['da1'] = new_df['da1'][:-1]
    
    new_df = pd.DataFrame(new_df)
    new_df['offset'] = new_df['nxt_start'] - new_df['end']
...
def main(indir, outdir):   
    ...
    for i, df in enumerate(tqdm(datas)):
        ...
        last_agent_end = []
        agent_end = -1
        for i in range(len(nxt_start)):
            last_agent_end.append(agent_end)
            if df['spk'].iloc[i] == 0: agent_end = df['end'].iloc[i]
        df['last_agent_end'] = last_agent_end
        """

        # 1 [ms]以下の発話を除去
        df = df[(df['end']-df['start']>1000)]
        
        # Userの短い発話(fil, la のみの発話)を削る
        df = df[df['da1']!='bc']
        df = df[df['nxt_da1']!='bc']
        df = df[df['da']!='fil']
        df = df[df['nxt_da']!='fil']
        ...
        # [System(1), User, System(2) の発話] かつ [System(1)の発話終了がUserの発話終了よりも後] を除く
        # df = df[df['last_agent_end']<=df['end']]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--indir', type=str, default='/mnt/aoni04/yaguchi/code/ResponseTimingEstimator_DA/data/ATR_Annotated/annotation_output', help='model type')
