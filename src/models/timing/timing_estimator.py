...
class TimingEstimator(nn.Module):
    ...
    def forward(self, inputs, input_lengths):
        ...
        # print('3', torch.tensor(outputs).shape)
    ...
    # 重み有Loss
    def get_loss2(self, probs, targets):
        N1 = torch.sum(targets)
        N0 = len(targets) - N1
        loss = self.criterion(probs[:N0], targets.float()[:N0]) * N1 / N0 + self.criterion(probs[N0:], targets.float()[N0:])
        # loss = self.criterion(probs[:N0], targets.float()[:N0]) + self.criterion(probs[N0:], targets.float()[N0:]) * N0 / N1
        return loss