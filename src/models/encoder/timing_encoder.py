    ...
    def __init__(self,
        ...
        if is_use_silence:
            ...
            if device == torch.device('cpu'):
                ...
                self.load_state_dict(torch.load(vad_model_path, map_location=torch.device(device)), strict=False)
        ...
        if is_use_n_word:
            ...
            if device == torch.device('cpu'):           
                ...
                self.lm.load_state_dict(torch.load(lm_model_path, map_location=torch.device(device))) #, strict=False)
    ...
    def streaming_inference(self, feats, idxs, input_lengths, indices, split, debug=False):
        ...
        if self.is_use_silence:
            ...
            for i in range(b):                
                ...
                #print('dialog_length:', dialog_length)              
                silences[i][:len(silence)] = silence
                utter_lengths[i][:len(utter_length)] = utter_length
                dialog_lengths[i][:len(dialog_length)] = dialog_length
